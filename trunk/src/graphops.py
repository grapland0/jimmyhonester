...
def erase_cycle_n_ntv(func):
	'''this routine will remove all arcs which transed less than 1 time.'''
	for ib in func.blocks.itervalues():
...
def check_DAG(func):
	'''this routine will remove all arcs or blocks that has not connected to entry block, 
	and remove cycle edge, but it remains probs which has been claced in calc_branch_p'''
	...
	for it in bid_used:
		del func.blocks[it]
	for ib in func.blocks.itervalues():
		ib.arcs=filter(lambda x:x.dest not in bid_used and ib.cycle==True,ib.arcs)
